...
| ③b | 超7天在 dashboard 人工审批关闭 | ✅ **已上线（2026-07-15，部署+核实）** | 后端新端点 `GET /v1/dashboard/stale-pending`（`unanswered_customer AND last_customer_at < now()-ALERT_MAX_WAIT_DAYS d`，与 ③a 同步的 7 天口径；跨客户、最久优先）；ticket-widget 加同名 `_passthrough` 代理路由。前端 `dashboard.js`：导航新增「超7天待审批」页，逐条列出（客户跳转 + 点进详情），每行内联「✓ 审批关闭」按钮（`canWrite` 才显示）→ 走现有 `/review` close→`closed_confirmed`，成功即移除该行。**实测 live 端点：24 条**（正好 = ③a 移出实时的 24 条），全部 >7 天 / unanswered / open，等待 9.8–44.7 天，0 违规。两容器 `docker cp`→widget `restart`→`commit` 固化。⚠️ 浏览器渲染 + 内联关闭点击需飞书登录，未 headless 验证（数据/路由/静态层已验证）。 |
